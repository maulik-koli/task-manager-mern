...
const ProjectContainer = ({ containerClass, containerButtons , fetchedProjectData, handleSave}) => {
    const { postCreatedData, dateResponse, setDataResponse, isDataLoading } = useContext(DataContext)
    const [projectData, setProjectData] = useState(fetchedProjectData)
    ...
    return (
        <>  
            {(isDataLoading && !projectData) ? <Loading /> :
                dateResponse ? <AlertMessage /> :
